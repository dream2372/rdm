...
from selfdrive.debug.dbc_dec_to_name import dec_to_name
...
def can_printer(bus=0, init_msgs=None, new_msgs=None, table=False, dbc=None, do_header=True):
  ...
  for addr in sorted(dat.keys()):
    ...
    name = addr
    if dbc is not None:
      name = dec_to_name(address=[addr], dbc=dbc)
      header = f"{name[0]} {hex(addr).ljust(6)}({str(addr).ljust(4)})"
    else:
...
if __name__ == "__main__":
  ...
  parser.add_argument("--dbc", type=str, nargs='?', help="Search for frame names within this DBC file.", default=None)
  parser.add_argument("--header", type=int, nargs='?', help="Search for frame names within this DBC file.", default=True)
  ...
  can_printer(args.bus, init_msgs=init_lr, new_msgs=new_lr, table=args.table, dbc=args.dbc, do_header=args.header)
