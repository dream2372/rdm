...
def get_radar_can_parser(CP):
  ...
  return CANParser(DBC[CP.carFingerprint]['radar'], messages, 0)
...
class RadarInterface(RadarInterfaceBase):
  ...
  def update(self, can_strings):
    ...
    # TODO: Put back sgufail once that's usable again
    # if sgu_info['RADC_HWFail'] or sgu_info['RADC_SGUFail'] or sgu_info['RADC_SensorDirty']:
    if sgu_info['RADC_HWFail'] or sgu_info['RADC_SensorDirty']:
      errors.append('fault')
    ret.errors = errors
    ...
    for i in range(NUM_POINTS):
      ...
      self.pts[i].yRel = msg_a['LatDist'] + 0.57 # For Civic Hatch
