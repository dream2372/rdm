...
def FakeHonda(pm, frame, idx, packer):
  can_sends = []

  ###### 100hz #####
  for bus in [0,2]:
    # can_sends.append(packer.make_can_msg("SRS_KINEMATICS", bus, {})) # 148
    can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_GAS_PEDAL_2", bus, {})) # 304
    # can_sends.append(packer.make_can_msg("EPS_STEERING_SENSORS_A", bus, {})) # 330
    can_sends.append(packer.make_can_msg("TCM_GATEWAYFORWARD_ENGINE_DATA", bus, {})) # 344
    # can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_POWERTRAIN_DATA", bus, {})) # 380
    # can_sends.append(packer.make_can_msg("TCM_GATEWAYFORWARD_GEARBOX_CVT", bus, {})) # 401
    if bus == 0:
      # bus 0
  ...
  if (frame % 2) == 0:
    for bus in [0,2]:
      ...
      # can_sends.append(packer.make_can_msg("VSA_GATEWAYFORWARD_EPB_STATUS", bus, {})) # 450
      ...
      if bus == 0:
        ...
        # can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_1DD", bus, {})) # 477
        can_sends.append(packer.make_can_msg("VSA_KINEMATICS", bus, {})) # 490
      else:
        can_sends.append(packer.make_can_msg("VSA_GATEWAYFORWARD_1AC", bus, {})) # 428
        can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_1D6", bus, {})) # 470
        can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_ENGINE_DATA_2", bus, {})) # 476
        can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_ACC_CONTROL_ON_2", bus, {})) # 493
  ...
  if (frame % 10) == 0:
    for bus in [0,2]:
      ...
      can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_320", bus, {})) # 800
      # can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_CRUISE", bus, {})) # 804
      # can_sends.append(packer.make_can_msg("METER_SCM_FEEDBACK", bus, {})) # 806
      can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_328", bus, {})) # 808
      ...
      if bus == 2:
        # can_sends.append(packer.make_can_msg("VSA_GATEWAYFORWARD_31B", bus, {})) # 795
  ...
  if (frame % 33) == 0:
    for bus in [0,2]:
      # can_sends.append(packer.make_can_msg("TCM_GATEWAYFORWARD_403", bus, {})) # 1027
      can_sends.append(packer.make_can_msg("METER_DOORS_STATUS", bus, {})) # 1029
      # can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_40C", bus, {})) # 1036
      # can_sends.append(packer.make_can_msg("TCM_GATEWAYFORWARD_40F", bus, {})) # 1039
      can_sends.append(packer.make_can_msg("NEW_MSG_445", bus, {})) # 1093
      can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_454", bus, {})) # 1108
    if bus == 0:
      can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_400", bus, {})) # 1024
    else:
  ...
  if (frame % 50) == 0:
    ...
    if bus == 0:
      ...
      can_sends.append(packer.make_can_msg("PCM_GATEWAYFORWARD_661", bus, {})) # 1633
    else:
      can_sends.append(packer.make_can_msg("OBD2_METER_510", bus, {})) # 1296

  ###### 1hz #####
  if (frame % 100) == 0:
    print(f"Time is {frame}")
